> **文档定位**：`TableFormatService` / 三线表实现的改进清单与风险点。与根目录 [README.md](../README.md)「三线表」摘要、[功能扩展研究报告](功能扩展研究报告.md) 功能表互补。

把「三线表」后续可优化的点按 **接口设计 / 实现细节 / 性能与可维护性 / 交互体验 / 易改动项** 五个维度梳理如下。
...
另：`ITableBatchService` 提供全稿等批量三线表能力；Ribbon 仅保留 `OnBatchThreeLineAll`（全稿前二次确认）。当前页 / 选中范围等仍可通过 `FormatCurrentSlideTables`、`FormatSelectedTables` 由代码调用；实现上复用 `FormatTableAsThreeLine`，与「三线表」单选入口一致。

**可扩展点**：

1. 多表处理的可见反馈：日志已有 “已对 X 个表格应用三线表格式”，可考虑在 X>1 时弹轻量提示；
2. 长耗时操作提示：全稿批处理可加“正在处理，请稍候”提示或提升日志级别。
...
> 与仓库首次生成/修复损坏配置时的默认模板一致，源码见 `PPA.Core.Configuration.PPAConfig.GetDefaultXmlContent()`。你本地 `PPAConfig.xml` 可与上表不同，仅作结构参考。
