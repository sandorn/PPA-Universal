# PPA 项目多版本分支规划方案

**版本**: v1.0  
**制定日期**: [当前日期]  
**状态**: 📋 方案设计阶段

---

## 📋 一、项目背景与目标

### 1.1 当前状态

PPA 项目当前是一个基于 VSTO 的 PowerPoint 插件，主要特点：

- **技术栈**: VSTO + NetOffice + .NET Framework 4.8
- **目标平台**: Microsoft PowerPoint
- **架构**: 已具备基础的抽象层设计（`Core/Abstraction/`）
- **DI 支持**: 已引入 `Microsoft.Extensions.DependencyInjection`

### 1.2 版本规划目标

将项目拆分为 **3 个独立构建版本**：

| 版本               | 名称            | 支持平台             | 优先级 |
| ------------------ | --------------- | -------------------- | ------ |
| **PPA.PowerPoint** | PowerPoint 专版 | Microsoft PowerPoint | 高     |
| **PPA.WPS**        | WPS 专版        | WPS 演示             | 中     |
| **PPA.Universal**  | 通用版          | PowerPoint + WPS     | 低     |

### 1.3 核心约束

- ✅ **最大化代码复用**：业务逻辑层共享
- ✅ **独立构建发布**：各版本可独立编译、打包、发布
- ✅ **并行开发**：团队可同时推进多个版本
- ✅ **最小化维护成本**：避免代码分叉和重复

---

## 🏗️ 二、架构设计

### 2.1 整体架构

采用 **"共享核心 + 平台适配器 + 版本壳"** 的三层架构：

```
┌─────────────────────────────────────────────────────────────┐
│                    版本壳层 (Entry Points)                   │
├───────────────────┬───────────────────┬─────────────────────┤
│  PPA.PowerPoint   │     PPA.WPS       │   PPA.Universal     │
│  (VSTO Add-in)    │  (COM/WPS插件)    │   (条件加载)        │
└───────────────────┴───────────────────┴─────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                 平台适配器层 (Adapters)                      │
├─────────────────────────────┬───────────────────────────────┤
│   PPA.Adapter.PowerPoint    │      PPA.Adapter.WPS          │
│   - PowerPointAppProvider   │      - WPSAppProvider         │
│   - NetOffice 实现          │      - COM dynamic 实现        │
└─────────────────────────────┴───────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                  共享核心层 (Shared Core)                    │
├─────────────────────────────────────────────────────────────┤
│  PPA.Core          - 抽象接口、DI、日志、异常处理            │
│  PPA.Business      - 格式化、批量操作、对齐等业务逻辑         │
│  PPA.UI.Shared     - 共享UI组件（Forms、资源）               │
└─────────────────────────────────────────────────────────────┘
```

### 2.2 项目结构

```
PPA/
├── src/
│   ├── Core/                           # 共享核心库
│   │   ├── PPA.Core/                   # 基础设施
│   │   │   ├── Abstraction/            # 平台无关接口
│   │   │   │   ├── IApplicationContext.cs
│   │   │   │   ├── ISlideOperations.cs
│   │   │   │   ├── IShapeOperations.cs
│   │   │   │   └── ITableOperations.cs
│   │   │   ├── DI/
│   │   │   ├── Logging/
│   │   │   └── ExHandler.cs
│   │   │
│   │   ├── PPA.Business/               # 业务逻辑（平台无关）
│   │   │   ├── Formatting/
│   │   │   │   ├── TableFormatService.cs
│   │   │   │   ├── TextFormatService.cs
│   │   │   │   └── ChartFormatService.cs
│   │   │   ├── Alignment/
│   │   │   │   └── AlignmentService.cs
│   │   │   └── Batch/
│   │   │       ├── TableBatchService.cs
│   │   │       └── ShapeBatchService.cs
│   │   │
│   │   └── PPA.UI.Shared/              # 共享UI组件
│   │       ├── Forms/
│   │       ├── Resources/
│   │       └── Ribbon/
│   │           └── RibbonDefinition.xml
│   │
│   ├── Adapters/                       # 平台适配器
│   │   ├── PPA.Adapter.PowerPoint/     # PowerPoint 适配器
│   │   │   ├── PowerPointContext.cs
│   │   │   ├── PowerPointSlideOps.cs
│   │   │   ├── PowerPointShapeOps.cs
│   │   │   └── PowerPointTableOps.cs
│   │   │
│   │   └── PPA.Adapter.WPS/            # WPS 适配器
│   │       ├── WPSContext.cs
│   │       ├── WPSSlideOps.cs
│   │       ├── WPSShapeOps.cs
│   │       └── WPSTableOps.cs
│   │
│   └── Hosts/                          # 版本入口
│       ├── PPA.PowerPoint/             # PowerPoint 专版 (VSTO)
│       │   ├── ThisAddIn.cs
│       │   ├── CustomRibbon.cs
│       │   └── PPA.PowerPoint.csproj
│       │
│       ├── PPA.WPS/                    # WPS 专版
│       │   ├── WPSAddIn.cs
│       │   ├── WPSRibbon.cs
│       │   └── PPA.WPS.csproj
│       │
│       └── PPA.Universal/              # 通用版
│           ├── UniversalAddIn.cs
│           ├── PlatformDetector.cs
│           └── PPA.Universal.csproj
│
├── tests/                              # 测试项目
│   ├── PPA.Core.Tests/
│   ├── PPA.Business.Tests/
│   └── PPA.Integration.Tests/
│
├── build/                              # 构建脚本
│   ├── build-powerpoint.ps1
│   ├── build-wps.ps1
│   ├── build-universal.ps1
│   └── build-all.ps1
│
└── PPA.sln                             # 解决方案文件
```

### 2.3 项目引用关系

```
PPA.PowerPoint ──────┐
                     ├──→ PPA.Adapter.PowerPoint ──→ PPA.Core
PPA.Universal ───────┤                               PPA.Business
                     ├──→ PPA.Adapter.WPS ─────────→ PPA.UI.Shared
PPA.WPS ─────────────┘
```

---

## 🔧 三、核心接口设计

### 3.1 应用上下文接口

```csharp
// PPA.Core/Abstraction/IApplicationContext.cs
namespace PPA.Core.Abstraction
{
    /// <summary>
    /// 平台无关的应用程序上下文接口
    /// </summary>
    public interface IApplicationContext
    {
        /// <summary>当前平台类型</summary>
        PlatformType Platform { get; }

        /// <summary>当前活动的演示文稿</summary>
        IPresentationContext ActivePresentation { get; }

        /// <summary>当前活动的窗口</summary>
        IWindowContext ActiveWindow { get; }

        /// <summary>当前选择</summary>
        ISelectionContext Selection { get; }

        /// <summary>检查特性是否支持</summary>
        bool IsFeatureSupported(Feature feature);
    }

    public enum PlatformType
    {
        Unknown = 0,
        PowerPoint = 1,
        WPS = 2
    }
}
```

### 3.2 形状操作接口

```csharp
// PPA.Core/Abstraction/IShapeOperations.cs
namespace PPA.Core.Abstraction
{
    /// <summary>
    /// 形状操作接口（平台无关）
    /// </summary>
    public interface IShapeOperations
    {
        /// <summary>获取形状位置</summary>
        ShapeRect GetBounds(object shape);

        /// <summary>设置形状位置</summary>
        void SetBounds(object shape, ShapeRect bounds);

        /// <summary>获取形状旋转角度</summary>
        float GetRotation(object shape);

        /// <summary>设置形状旋转角度</summary>
        void SetRotation(object shape, float angle);

        /// <summary>判断是否为表格</summary>
        bool IsTable(object shape);

        /// <summary>判断是否为图表</summary>
        bool IsChart(object shape);
    }

    public struct ShapeRect
    {
        public float Left { get; set; }
        public float Top { get; set; }
        public float Width { get; set; }
        public float Height { get; set; }
    }
}
```

### 3.3 表格操作接口

```csharp
// PPA.Core/Abstraction/ITableOperations.cs
namespace PPA.Core.Abstraction
{
    /// <summary>
    /// 表格操作接口（平台无关）
    /// </summary>
    public interface ITableOperations
    {
        /// <summary>获取表格行数</summary>
        int GetRowCount(object table);

        /// <summary>获取表格列数</summary>
        int GetColumnCount(object table);

        /// <summary>获取单元格</summary>
        object GetCell(object table, int row, int col);

        /// <summary>设置单元格文本</summary>
        void SetCellText(object cell, string text);

        /// <summary>设置单元格背景色</summary>
        void SetCellBackground(object cell, int colorRgb);

        /// <summary>设置边框样式</summary>
        void SetBorderStyle(object cell, BorderEdge edge, BorderStyle style);
    }
}
```

---

## 📦 四、DI 服务注册策略

### 4.1 核心服务注册（共享）

```csharp
// PPA.Core/DI/CoreServiceExtensions.cs
namespace PPA.Core.DI
{
    public static class CoreServiceExtensions
    {
        /// <summary>
        /// 注册核心服务（所有版本共享）
        /// </summary>
        public static IServiceCollection AddPPACore(this IServiceCollection services)
        {
            // 配置服务
            services.AddSingleton<IFormattingConfig>(sp => FormattingConfig.Instance);

            // 日志服务
            services.AddSingleton<ILogger, ProfilerLoggerAdapter>();

            // 资源管理
            services.AddSingleton<IResourceManager, ResourceManager>();

            return services;
        }

        /// <summary>
        /// 注册业务服务（所有版本共享）
        /// </summary>
        public static IServiceCollection AddPPABusiness(this IServiceCollection services)
        {
            // 格式化服务
            services.AddTransient<ITableFormatService, TableFormatService>();
            services.AddTransient<ITextFormatService, TextFormatService>();
            services.AddTransient<IChartFormatService, ChartFormatService>();

            // 批量操作服务
            services.AddTransient<ITableBatchService, TableBatchService>();
            services.AddTransient<IShapeBatchService, ShapeBatchService>();

            // 对齐服务
            services.AddTransient<IAlignmentService, AlignmentService>();

            return services;
        }
    }
}
```

### 4.2 PowerPoint 适配器注册

```csharp
// PPA.Adapter.PowerPoint/DI/PowerPointAdapterExtensions.cs
namespace PPA.Adapter.PowerPoint.DI
{
    public static class PowerPointAdapterExtensions
    {
        public static IServiceCollection AddPowerPointAdapter(
            this IServiceCollection services)
        {
            // 注册 PowerPoint 特定实现
            services.AddSingleton<IApplicationContext, PowerPointContext>();
            services.AddSingleton<IShapeOperations, PowerPointShapeOps>();
            services.AddSingleton<ITableOperations, PowerPointTableOps>();
            services.AddSingleton<ISlideOperations, PowerPointSlideOps>();

            return services;
        }
    }
}
```

### 4.3 WPS 适配器注册

```csharp
// PPA.Adapter.WPS/DI/WPSAdapterExtensions.cs
namespace PPA.Adapter.WPS.DI
{
    public static class WPSAdapterExtensions
    {
        public static IServiceCollection AddWPSAdapter(
            this IServiceCollection services)
        {
            // 注册 WPS 特定实现
            services.AddSingleton<IApplicationContext, WPSContext>();
            services.AddSingleton<IShapeOperations, WPSShapeOps>();
            services.AddSingleton<ITableOperations, WPSTableOps>();
            services.AddSingleton<ISlideOperations, WPSSlideOps>();

            return services;
        }
    }
}
```

### 4.4 各版本入口配置

```csharp
// PPA.PowerPoint/ThisAddIn.cs - PowerPoint 专版
private void InitializeDIContainer()
{
    var services = new ServiceCollection();
    services.AddPPACore();
    services.AddPPABusiness();
    services.AddPowerPointAdapter();  // PowerPoint 适配器
    _serviceProvider = services.BuildServiceProvider();
}

// PPA.WPS/WPSAddIn.cs - WPS 专版
private void InitializeDIContainer()
{
    var services = new ServiceCollection();
    services.AddPPACore();
    services.AddPPABusiness();
    services.AddWPSAdapter();  // WPS 适配器
    _serviceProvider = services.BuildServiceProvider();
}

// PPA.Universal/UniversalAddIn.cs - 通用版
private void InitializeDIContainer()
{
    var services = new ServiceCollection();
    services.AddPPACore();
    services.AddPPABusiness();

    // 运行时检测平台
    var platform = PlatformDetector.Detect();
    switch (platform)
    {
        case PlatformType.PowerPoint:
            services.AddPowerPointAdapter();
            break;
        case PlatformType.WPS:
            services.AddWPSAdapter();
            break;
        default:
            throw new PlatformNotSupportedException();
    }

    _serviceProvider = services.BuildServiceProvider();
}
```

---

## 🔄 五、版本构建策略

### 5.1 条件编译符号

| 项目           | 编译符号     |
| -------------- | ------------ |
| PPA.PowerPoint | `POWERPOINT` |
| PPA.WPS        | `WPS`        |
| PPA.Universal  | `UNIVERSAL`  |

### 5.2 项目文件配置示例

```xml
<!-- PPA.PowerPoint.csproj -->
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <TargetFramework>net48</TargetFramework>
    <OutputType>Library</OutputType>
    <DefineConstants>POWERPOINT;VSTO40</DefineConstants>
  </PropertyGroup>

  <ItemGroup>
    <ProjectReference Include="..\..\Core\PPA.Core\PPA.Core.csproj" />
    <ProjectReference Include="..\..\Core\PPA.Business\PPA.Business.csproj" />
    <ProjectReference Include="..\..\Adapters\PPA.Adapter.PowerPoint\PPA.Adapter.PowerPoint.csproj" />
  </ItemGroup>
</Project>
```

```xml
<!-- PPA.WPS.csproj -->
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <TargetFramework>net48</TargetFramework>
    <OutputType>Library</OutputType>
    <DefineConstants>WPS</DefineConstants>
  </PropertyGroup>

  <ItemGroup>
    <ProjectReference Include="..\..\Core\PPA.Core\PPA.Core.csproj" />
    <ProjectReference Include="..\..\Core\PPA.Business\PPA.Business.csproj" />
    <ProjectReference Include="..\..\Adapters\PPA.Adapter.WPS\PPA.Adapter.WPS.csproj" />
  </ItemGroup>
</Project>
```

### 5.3 构建脚本

```powershell
# build/build-all.ps1
param(
    [string]$Configuration = "Release"
)

Write-Host "构建 PPA 所有版本..." -ForegroundColor Cyan

# 构建共享库
dotnet build src/Core/PPA.Core/PPA.Core.csproj -c $Configuration
dotnet build src/Core/PPA.Business/PPA.Business.csproj -c $Configuration

# 构建适配器
dotnet build src/Adapters/PPA.Adapter.PowerPoint/PPA.Adapter.PowerPoint.csproj -c $Configuration
dotnet build src/Adapters/PPA.Adapter.WPS/PPA.Adapter.WPS.csproj -c $Configuration

# 构建各版本
dotnet build src/Hosts/PPA.PowerPoint/PPA.PowerPoint.csproj -c $Configuration
dotnet build src/Hosts/PPA.WPS/PPA.WPS.csproj -c $Configuration
dotnet build src/Hosts/PPA.Universal/PPA.Universal.csproj -c $Configuration

Write-Host "构建完成！" -ForegroundColor Green
```

---

## 📊 六、代码复用分析

### 6.1 复用率估算

| 层级           | 代码量占比 | 复用率           |
| -------------- | ---------- | ---------------- |
| **共享核心层** | ~60%       | 100%             |
| **业务逻辑层** | ~25%       | 100%             |
| **平台适配器** | ~10%       | 0%（各平台独立） |
| **版本入口**   | ~5%        | ~30%             |
| **总体**       | 100%       | **~85%**         |

### 6.2 各层职责

| 层级               | 职责                     | 代码特点               |
| ------------------ | ------------------------ | ---------------------- |
| **PPA.Core**       | 抽象接口、DI、日志、异常 | 纯抽象，无平台依赖     |
| **PPA.Business**   | 格式化、对齐、批量操作   | 依赖抽象接口，平台无关 |
| **PPA.UI.Shared**  | Forms、资源、Ribbon 定义 | 共享 UI 组件           |
| **PPA.Adapter.\*** | 平台 API 封装            | 直接调用平台 COM API   |
| **PPA.Host.\***    | 插件入口、DI 配置        | 平台特定初始化         |

---

## 📅 七、实施路线图

### 阶段一：架构重构（2-3 周）

**目标**：将现有代码重构为分层架构

| 任务                        | 工作量 | 状态      |
| --------------------------- | ------ | --------- |
| 创建新解决方案结构          | 2 天   | ✅ 已完成 |
| 提取 PPA.Core 项目          | 3 天   | ✅ 已完成 |
| 提取 PPA.Business 项目      | 5 天   | ✅ 已完成 |
| 实现 PPA.Adapter.PowerPoint | 3 天   | ✅ 已完成 |
| 创建 PPA.PowerPoint 入口    | 2 天   | ✅ 已完成 |
| 验证 PowerPoint 版本功能    | 2 天   | ✅ 已完成 |

### 阶段二：WPS 适配器开发（3-4 周）

**目标**：实现 WPS 平台支持

| 任务                   | 工作量 | 状态      |
| ---------------------- | ------ | --------- |
| WPS COM API 研究与验证 | 3 天   | ✅ 已完成 |
| 实现 PPA.Adapter.WPS   | 7 天   | ✅ 已完成 |
| 创建 PPA.WPS 入口      | 3 天   | ✅ 已完成 |
| WPS 功能适配与测试     | 5 天   | ✅ 已完成 |
| 处理平台差异和降级     | 3 天   | ✅ 已完成 |

### 阶段三：通用版开发（1-2 周）

**目标**：实现运行时平台检测的通用版本

| 任务                    | 工作量 | 状态      |
| ----------------------- | ------ | --------- |
| 实现 PlatformDetector   | 2 天   | ✅ 已完成 |
| 创建 PPA.Universal 入口 | 2 天   | ✅ 已完成 |
| 动态加载适配器机制      | 3 天   | ✅ 已完成 |
| 集成测试                | 3 天   | ✅ 已完成 |

### 阶段四：测试与发布（2 周）

**目标**：完善测试，准备发布

| 任务         | 工作量 | 状态      |
| ------------ | ------ | --------- |
| 单元测试补充 | 3 天   | ✅ 已完成 |
| 集成测试     | 3 天   | ✅ 已完成 |
| 打包脚本编写 | 2 天   | ✅ 已完成 |
| 文档更新     | 2 天   | ✅ 已完成 |

---

## ⚠️ 八、风险与应对

### 8.1 技术风险

| 风险                   | 影响 | 应对策略                         |
| ---------------------- | ---- | -------------------------------- |
| WPS COM API 兼容性不足 | 高   | 优先验证核心 API；实现功能降级   |
| 性能开销               | 中   | 抽象层使用轻量接口；关键路径优化 |
| 重构引入回归           | 中   | 保持测试覆盖；分步重构验证       |

### 8.2 项目风险

| 风险          | 影响 | 应对策略                             |
| ------------- | ---- | ------------------------------------ |
| 工作量超预期  | 中   | 分阶段交付；优先保证 PowerPoint 版本 |
| 平台 API 变更 | 低   | 适配器层隔离变化；版本兼容处理       |

---

## 🎯 九、决策建议

### 9.1 推荐实施顺序

1. **第一优先级**：完成架构重构，确保 PowerPoint 版本稳定
2. **第二优先级**：开发 WPS 适配器，验证技术可行性
3. **第三优先级**：实现通用版本（可选，视需求决定）

### 9.2 版本策略建议

| 场景                   | 推荐版本       | 理由                 |
| ---------------------- | -------------- | -------------------- |
| 企业用户（Office 365） | PPA.PowerPoint | 最佳兼容性和功能支持 |
| 企业用户（WPS）        | PPA.WPS        | 针对性优化           |
| 个人用户（混合环境）   | PPA.Universal  | 自动适配             |

### 9.3 简化方案（备选）

如果资源有限，可以考虑 **两版本方案**：

- **PPA.PowerPoint**：保持现状，持续维护
- **PPA.Universal**：通用版本，通过运行时检测支持两平台

这样可以减少维护成本，同时满足多平台需求。

---

## 📝 十、总结

本方案通过 **"共享核心 + 平台适配器 + 版本壳"** 的架构设计，实现：

1. ✅ **~85% 代码复用率** - 业务逻辑完全共享
2. ✅ **独立版本构建** - 各版本可独立发布
3. ✅ **并行开发支持** - 适配器层解耦，团队可并行工作
4. ✅ **可扩展架构** - 未来可轻松支持其他平台

**建议**：先完成架构重构并稳定 PowerPoint 版本，再逐步开发 WPS 支持。

---

**文档版本**: v1.0  
**维护者**: PPA 开发团队
