...
namespace PPA
{
	...
	public partial class ThisAddIn
	{
		...
		private PPAUniversal _ppaUniversal; // 新架构入口
		...
		/// 获取新架构入口（用于服务访问）
		/// </summary>
		public PPAUniversal Universal => _ppaUniversal;
		...
		private void InitializeUniversalFramework()
		{
			try
			{
				Logger.LogInformation("正在初始化新架构...");
				_ppaUniversal = new PPAUniversal();
				_ppaUniversal.Startup(NetApp, PPA.Core.Abstraction.PlatformType.PowerPoint);
				Logger.LogInformation($"新架构初始化完成，平台: {_ppaUniversal.Platform}");

				// 初始化服务桥接器，让旧代码可以访问新架构服务
				LegacyServiceBridge.Initialize(_ppaUniversal.ServiceProvider);
				Logger.LogInformation("服务桥接器初始化完成");
			}
			catch (Exception ex)
			{
				Logger.LogWarning($"新架构初始化失败（不影响旧功能）: {ex.Message}");
				_ppaUniversal = null;
			}
		}
